...
`.mqp` 是 MaoqiuPlayer 的专用媒体包格式。底层格式使用 `MAOQIU_PLAYER_ENC_V1` 作为文件识别头，并通过标准加密库保护包内媒体数据。桌面端同时兼容 Maoqiu Encryptor 生成的 `MAOLITE1\0` 轻量包和 `MAOQIU1\0` 用户名/密码加密文件；移动端支持打开 `MAOLITE1\0` 轻量包。
...
推送到 GitHub `main` 分支后，GitHub Actions 会自动运行 `Package MaoqiuPlayer` 工作流，构建并上传临时 artifacts。推送 `v*` 标签后，工作流会创建 GitHub Release、写入更新内容并上传正式发布包：

- `MaoqiuPlayer-macOS.dmg`，打开后双击 `MaoqiuPlayer-macOS.pkg` 安装到 `/Applications`
- `MaoqiuPlayer-Windows-Setup.exe`
- `maoqiu-player_0.1.17_amd64.deb`
...
MaoqiuPlayer 使用 Qt Multimedia 的 FFmpeg 后端播放视频。不同平台、显卡驱动和视频编码组合可能影响播放效果；某些 HEVC/H.265、AV1、10-bit/HDR 或特殊 MKV 文件可能出现无法解码、只有声音或黑屏。

应用默认启用更保守的兼容播放配置，并在播放页显示后端错误提示。遇到不兼容文件时，可以先点击“系统播放器”用系统默认播放器打开。
